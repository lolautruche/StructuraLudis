...
export default function ExhibitionRolesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [exhibitionRes, rolesRes, zonesRes, usersRes] = await Promise.all([
  ...
  // Filter users for dropdown (exclude those already with roles)
  const availableUsers = users.filter(
    (user) =>
      !roles.some((role) => role.user_id === user.id) &&
      (userSearch === '' ||
        user.email.toLowerCase().includes(userSearch.toLowerCase()) ||
        (user.full_name?.toLowerCase().includes(userSearch.toLowerCase()) ?? false))
  );
